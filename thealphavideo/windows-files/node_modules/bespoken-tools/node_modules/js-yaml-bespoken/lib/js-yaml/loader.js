...
function readPlainScalar(state, nodeIndent, withinFlowCollection) {
  ...
  if (ch === 0x3F/* ? */ || ch === 0x2D/* - */) {
    ...
    // If this is a child (indented), then we verify if it start with dash, and it should have an space afterwards
    if (nodeIndent === 1 && ch === 0x2D /* - */) {
      if (state.input.charCodeAt(state.position) === 0x2D &&
        state.input.charCodeAt(state.position + 1) !== 0x20/* Space */
      ) {
        throwError(state, 'syntax error: a starting hyphen (-) inside a test has to be followed by a blank space');
      }
    }
  }
